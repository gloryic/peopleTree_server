...
viewTree = {
  init : function(id) {
    ...
    viewTree.getTreeData(id,0);
  },

  getTreeData : function(ID,time){
    setTimeout(function() {
        $.ajax({
          type:"GET",
              url:'http://210.118.74.107:3000/ptree/util/showTreeV2?rootGroupMemberId='+ID,
              complete: function (data){
                myDiagram.model = new go.TreeModel(data.responseJSON);
                viewTree.getTreeData(ID,2000);
              },
              error : function(data){
